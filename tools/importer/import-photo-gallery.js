...
function createPhotoGallery(doc) {
  const swiperGallery = doc.querySelector('article.swiper-gallery');
  if (!swiperGallery) {
    return;
  }

  const images = swiperGallery.querySelectorAll(':scope img');
  if (images.length === 0) {
    return;
  }
